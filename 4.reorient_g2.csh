...
foreach ind (`seq 1 1 $n`)
	...
	set first=`awk -v line=$ind -v col=1 'NR == '$ind' { print $1 }' < rsfmri/restingstate_scan_number_g2_temp.lst` # change to restingstate_scan_number.lst for real analysis
	set second=`awk -v line=$ind -v col=2 'NR == '$ind' { print $2 }' < rsfmri/restingstate_scan_number_g2_temp.lst` # change to restingstate_scan_number.lst for real analysis
	set third=`awk -v line=$ind -v col=3 'NR == '$ind' {print $3 }' < rsfmri/restingstate_scan_number_g2_temp.lst` # change to restingstate_scan_number.lst for real analysis
